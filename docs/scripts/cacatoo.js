 ...
 * @returns {{ data: Float64Array, size: number }}
 */
function makeDiskKernel(radius) {
 ...
 */
function applyKernelFFT(gridmodel, readProp, writeProp, kernelObj, scale = 1) {
...
/**
    ...
    castDiskFFT(sourceState, sourceValue, targetState, amount, kernel, radius) {
        ...
        applyKernelFFT(this, srcProp, resProp, kernel, amount);
        for (let x = 0; x < this.nc; x++)
            for (let y = 0; y < this.nr; y++) {
